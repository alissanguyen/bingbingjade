...
function TagDot({
  ...
  // Desktop: open on hover. Mobile: open when this tag is active.
  ...
  return (
    ...
    >
      ...
      {/* Desktop only: absolute card, edge-aware, hover-controlled */}
      {!isMobile && open && (
        <div
      ...
      {/*
        Mobile card is intentionally NOT rendered here.
        This wrapper has transform: translate(-50%, -50%) which makes position:fixed children
        anchor to this element instead of the viewport, causing the card to appear clipped
        near the dot. The mobile overlay is rendered at CollectionScene level instead.
      */}
    </div>
  );
}
...
export function CollectionScene({ scene }: Props) {
  ...
  // Resolved only on mobile; drives the fixed overlay rendered below.
  const activeTag = isMobile && activeTagId
    ? scene.tags.find((t) => t.id === activeTagId) ?? null
    : null;
  ...
  return (
    <figure className="relative group hover:z-10">
      ...
      {/* Mobile overlay: fixed to the viewport bottom, outside TagDot's transformed container
          so position:fixed resolves to the viewport rather than the dot element. */}
      {activeTag && (
        <div
          className="fixed bottom-4 left-1/2 -translate-x-1/2 w-56 z-[9999] pointer-events-auto"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="relative">
            <TagCard product={activeTag.products} />
            <button
              type="button"
