...
import { buildCareTipsHtml } from "@/lib/email-templates";
...
export async function POST(req: NextRequest) {
  ...
  if (preview) {
    const html = buildCareTipsHtml({ customerName: orders[0].customer_name });
  ...
  const subject = "Caring for your BingBing Jade piece";
    ...
    try {
      await resend.batch.send(
        chunk.map((o) => ({
          ...
          to: o.customer_email!,
          subject,
          html: buildCareTipsHtml({ customerName: o.customer_name }),
        }))
      );
      sent += chunk.length;
    } catch {
