...
export async function POST(req: NextRequest) {
  ...
  const { error: updateErr } = await supabaseAdmin
    ...
    .update({ payment_method: into.trim() })
    .eq("payment_method", from.trim());
  if (updateErr) return NextResponse.json({ error: updateErr.message }, { status: 500 });

  await supabaseAdmin.from("expense_payment_methods").delete().eq("name", from.trim());

  return NextResponse.json({ ok: true });
}

// DELETE ?name=X  — remove a method from the list without touching expenses
export async function DELETE(req: NextRequest) {
  ...
  await supabaseAdmin.from("expense_payment_methods").delete().eq("name", name);
  return NextResponse.json({ ok: true });
}
