...
export async function PATCH(
  ...
  for (const f of [
    "expense_date", "vendor", "category", "amount_usd",
    "payment_method", "receipt_url", "business_use_percent", "notes",
  ] as const) {
...
export async function DELETE(
  ...
  const { id } = await params;
  const { error } = await supabaseAdmin.from("business_expenses").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
