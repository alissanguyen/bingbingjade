...
export async function GET(req: NextRequest) {
  ...
  const rows = (orders ?? []).map((o) => {
    const fc =
      ((o.order_fulfillment_costs ?? []) as Record<string, unknown>[])[0] ??
      null;
    return {
      order_id: o.id,
  ...
  // Aggregate label cost across the full date range (not just this page)
  let labelQuery = supabaseAdmin
    .from("orders")
    .select("order_fulfillment_costs(label_cost_usd)")
    .neq("order_status", "order_cancelled")
    .not("amount_total", "is", null);
  if (from) labelQuery = labelQuery.gte("created_at", from);
  if (to) labelQuery = labelQuery.lte("created_at", to + "T23:59:59Z");
  const { data: labelOrders } = await labelQuery;
  const totalLabelCost = (labelOrders ?? []).reduce((sum, o) => {
    const fc = (
      (o.order_fulfillment_costs ?? []) as Record<string, unknown>[]
    )[0];
    return sum + Number(fc?.label_cost_usd ?? 0);
  }, 0);
...
export async function POST(req: NextRequest) {
  ...
  const {
    order_id,
    label_cost_usd,
    business_shipping_insurance_cost_usd,
    supplies_cost_usd,
    dropoff_transport_cost_usd,
    other_fulfillment_cost_usd,
    notes
  } = body as {
    order_id: string;
  ...
  const payload = {
    ...
    business_shipping_insurance_cost_usd:
      business_shipping_insurance_cost_usd ?? 0,
  ...
  // Check if a row already exists for this order
